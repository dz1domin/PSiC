...
const std::string rootdir = "../Strona";
const std::string success = "HTTP/1.1 200 OK\r\n";
const std::string failure = "HTTP/1.1 404 Not Found\r\nContent-type:text/html;charset=utf-8\r\nContent-length:0\r\n\r\n";
...
std::vector<char> parseData(std::string& data)
{
	...
	if(method == "POST")
	{
		...
		catch(...)
		{
			...
			return std::vector<char>(headers.begin(), headers.end());
		}
		...
		std::vector<char> v(headers.begin(), headers.end());
...
int main()
{
	...
	std::map<int, std::vector<char> > data_to_send;
		...
		if(poll(polling_cont.data(), polling_cont.size(), -1) != -1)
		{
			for(unsigned i = 0; i < polling_cont.size(); i++)
			{
				...
				if (i != 0 && polling_cont[i].revents & POLLIN)
				{
					...
					std::string data = polled_data[client_fd];
